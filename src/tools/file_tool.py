...
class FileSaveTool(Tool):
    ...
    Examples:
        ...
        >>> file_saver = FileSaveTool(backup_existing=True)
        >>> result = file_saver("./output.txt", "Hello, World!")
        >>> print(result)
        ```
    """
    ...
    def __init__(self, auto_create_dirs: bool = True, backup_existing: bool = False, encoding: str = "utf-8"):
        super().__init__()
        self.auto_create_dirs = auto_create_dirs
        self.backup_existing = backup_existing
        self.encoding = encoding
    
    def forward(self, content: str, file_path: str = None) -> str:
        # 如果没有提供文件路径，自动生成
        if file_path is None:
            random_number = str(uuid.uuid4().hex[:8])  # 使用8位随机字符
            file_path = f"tmp/{random_number}.json"
        
        logger.info(f"开始保存文件: {file_path}, 内容长度: {len(content)} 字符")
        ...
        try:
            ...
            # 备份现有文件
            backup_info = ""
            if self.backup_existing and os.path.exists(file_path):
                ...
                logger.info(f"备份现有文件: {file_path} -> {backup_path}")
                import shutil
            ...
            # 获取保存后的文件信息
            file_size = os.path.getsize(file_path)
            logger.success(f"文件保存成功: {file_path}, 文件大小: {file_size} 字节")
            
            return f"## 文件保存成功\n\n**文件路径:** {file_path}\n**文件大小:** {file_size} 字节\n**内容长度:** {len(content)} 字符{backup_info}"
            
        except PermissionError as e:
            logger.error(f"权限不足，无法写入文件: {file_path}, 错误: {e}")
            return f"错误：没有权限写入文件 - {file_path}"
        except OSError as e:
            logger.error(f"无法创建目录或文件: {file_path}, 错误: {e}")
            return f"错误：无法创建目录或文件 - {str(e)}"
        except Exception as e:
            logger.error(f"保存文件时发生未知异常: {file_path}, 错误: {e}")
